...
var Categories=['Politics','Entertainment','Technology','Health','Sports','International','Business','Lifestyle'];
        ...
        await page.waitForNavigation({ waitUntil: 'load' }) //networkidle0
         ...
         switch(Category){
             case "Politics" :
               ...
               PageData = await page.evaluate((Category)=>{
                       var titles = document.querySelectorAll('.ContentRoll__Headline>h2>a.AnchorLink');
                       var images =document.querySelectorAll('.Image__Wrapper>picture>img');
                       var time = document.querySelectorAll('.ContentRoll__TimeStamp .TimeStamp__Date ContentRoll__Date--recent')
                       var data =[];
                       
                       for(let j=0;j<titles.length;j++){
                        if(WordExist(time[j])==true){
                            data.push({
